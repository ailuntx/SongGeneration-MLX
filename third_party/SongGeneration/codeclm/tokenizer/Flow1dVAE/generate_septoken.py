...
def inference_autocast(device, dtype=torch.float16):
    device_type = torch.device(device).type
    if device_type in ("cuda", "mps"):
        return torch.autocast(device_type=device_type, dtype=dtype)
    return nullcontext()

def autocast_for_device(dtype=torch.float16):
    def decorator(fn):
        def wrapped(self, *args, **kwargs):
            with inference_autocast(self.device, dtype=dtype):
...
class Tango:
    ...
    @torch.no_grad()
    @autocast_for_device(dtype=torch.float32)
    ...
    def code2sound(self, codes, prompt_vocal=None, prompt_bgm=None, duration=40, guidance_scale=1.5, num_steps=20, disable_progress=False, chunked=False, chunk_size=128):
        ...
        with inference_autocast(self.device, dtype=torch.float16):
        ...
        elif torch.device(self.device).type == "mps" and hasattr(torch, "mps"):
