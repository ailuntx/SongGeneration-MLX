...
class CodecLM:
    ...
    def __init__(self, name: str, audiotokenizer: AudioTokenizer, lm: LmModel,
        ...
        elif self.seperate_tokenizer is not None and hasattr(self.seperate_tokenizer, "model"):
            self.device = torch.device(getattr(self.seperate_tokenizer.model, "device", "cpu"))
        elif self.audiotokenizer is not None and hasattr(self.audiotokenizer, "model"):
            self.device = torch.device(getattr(self.audiotokenizer.model, "device", "cpu"))
        else:
